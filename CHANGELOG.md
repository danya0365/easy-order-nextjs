...
- **ล็อกโหมด kiosk ไว้ที่หน้าสั่งอาหาร (URL containment)** — เดิมลูกค้าบนเครื่อง kiosk พิมพ์ URL
  ...
  เด้งทุกเส้นทางในแอปกลับไป `/kiosk` เมื่อมี cookie `eo_kiosk` (ความปลอดภัยการสั่งของเดิมแน่นอยู่แล้ว
  ผ่าน `requireKioskShop` — นี่คือการกันการนำทางออกนอกจอสั่ง). กันลูปกรณี cookie ค้าง: ปุ่มออกใน
  หน้า "ยังไม่ได้เปิดโหมด" + `exitKioskAction` เคลียร์ cookie ก่อนไป `/login`. **นอกขอบเขต**: การไป
  โดเมนอื่น (ต้องใช้ OS/MDM kiosk lock — ทำจากเว็บไม่ได้)
