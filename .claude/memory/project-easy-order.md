...
**⏳ STILL PENDING (the parked next item):** Public homepage **map** + `/shops` **directory**. Easy Stamp's generic shop map + directory were dropped by mistake during the clone (deps `maplibre-gl`/`react-map-gl`, `branchRepository.listMapLocations()`, i18n, and `tests/e2e/smoke.spec.ts` assertions left dangling/failing). To re-point to Easy Order; `CustomerTabBar`'s Map/Shops tabs were removed and return with it. (The directory can't be a verbatim port — Easy Order has no reviews/categories/shop-images.)
